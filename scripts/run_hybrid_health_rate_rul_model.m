%RUN_HYBRID_HEALTH_RATE_RUL_MODEL Compare baseline, feature and hybrid RUL models.
...
nestedFeaturePredictions = addModelLabel(evaluateFeatureSimilarityRulModel(outerRaceFeatures, nestedFolds, ...
    featureNames), "feature-similarity model");
nestedHybridPredictions = addModelLabel(evaluateHybridHealthRateRulModel(outerRaceFeatures, nestedFolds), ...
    "hybrid health-rate model");
conditionAgePredictions = addModelLabel(evaluateAgeOnlyRulBaseline(outerRaceFeatures, conditionFolds), ...
    "age-only baseline");
conditionFeaturePredictions = addModelLabel(evaluateFeatureSimilarityRulModel(outerRaceFeatures, conditionFolds, ...
    featureNames), "feature-similarity model");
conditionHybridPredictions = addModelLabel(evaluateHybridHealthRateRulModel(outerRaceFeatures, conditionFolds), ...
    "hybrid health-rate model");

predictions = [nestedAgePredictions; nestedFeaturePredictions; nestedHybridPredictions; ...
    conditionAgePredictions; conditionFeaturePredictions; conditionHybridPredictions];
...
outDir = fullfile(projectRoot, "results", "hybrid_health_rate_rul");
if ~isfolder(outDir)
    mkdir(outDir);
end

writetable(predictions, fullfile(outDir, "outer_race_hybrid_health_rate_predictions.csv"));
writetable(perBearingMetrics, fullfile(outDir, "outer_race_hybrid_health_rate_per_bearing_metrics.csv"));
writetable(summaryMetrics, fullfile(outDir, "outer_race_hybrid_health_rate_summary_metrics.csv"));
plotRepresentativeBearing(predictions, outDir);

disp(summaryMetrics);
fprintf("Wrote %s\n", fullfile(outDir, "outer_race_hybrid_health_rate_predictions.csv"));
fprintf("Wrote %s\n", fullfile(outDir, "outer_race_hybrid_health_rate_per_bearing_metrics.csv"));
fprintf("Wrote %s\n", fullfile(outDir, "outer_race_hybrid_health_rate_summary_metrics.csv"));
fprintf("Wrote %s\n", fullfile(outDir, "nested_bearing3_1_hybrid_rul_comparison.png"));
...
for modelIdx = 1:numel(models)
    ...
    for protocolIdx = 1:numel(protocols)
        ...
        predictionRows = predictions(predictions.Model == models(modelIdx) & ...
            predictions.Protocol == protocols(protocolIdx), :);
...
ylabel("RUL minutes");
title("Nested held-out Bearing3_1 hybrid health-rate RUL comparison");
legend("Location", "best");
exportgraphics(figureHandle, fullfile(outDir, "nested_bearing3_1_hybrid_rul_comparison.png"), Resolution=160);
close(figureHandle);
end
